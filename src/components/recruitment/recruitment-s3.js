...
function RecruitmentTimeline() {
  ...
  return (
    <Timeline className="md:mx-12" position={isMobile ? "right" : "alternate"}>
      <TimelineItem>
      ...
      {timelineData.map((timelineItem, index) => (
        <TimelineItem>
          {(isMobile ?  <h6 className="mt-2 font-bold text-green-500">
              {timelineItem.date}
            </h6> : <TimelineOppositeContent>
          ...
          <TimelineContent>
            <div className={`flex  ${(index % 2 != 0 || isMobile) ? "flex-row" : "flex-row-reverse"}`}>
            <div
                className={`mt-3 w-0 h-0 
  border-t-[8px] border-t-transparent
  ${(index % 2 != 0 || isMobile) ? "border-r-[7px] border-r-green-500" : "border-l-[7px] border-l-green-500"}
  border-b-[8px] border-b-transparent`}
              ...
              <div class="bg-green-500 rounded w-full">
                ...
                >
                  <h6 className="text-lg text-green-500">{timelineItem.title}</h6>
